...
- [2026-09-06] **[已修复，真 bug] 行间距过大的真正根因是文字悬空、不是间距算法本身**：用户发截图说"gap too much"。排查发现 `REF_FRONT_NUM` 一直挂在自己面板下方 307.8mm（`REF_BACK_NUM`/`REF_BACK_NAME` 也分别挂了 230.6mm/140mm），这是从最早那个手搭的参考件遗留下来、一直没修的问题（`_apply_one`/`resize_jersey_row` 的重新居中一直只改水平位置，从没检查垂直是否合理）。`duplicate_jersey_rows` 的 20mm 间距是按"整行所有形状的真实最低点"算的，算法本身没错，但这个最低点被悬空的文字拖得离面板本体几百 mm 远，于是"20mm 规则"被正确遵守了，面板与面板之间却空出了 300+mm。已加 `_snap_text_to_panel_bottom()`：任何文字如果底边低于自己所在面板的底边，就把它整体上移到与面板底边齐平（尺寸完全不变，只挪位置），接入 `resize_jersey_row`/`recenter_jersey_texts` 两处——前者保证以后每次生成新球衣都会自动修正，不会再复发；后者用于把已生成的 18 行一次性修好。
- [2026-09-06] **[已实现] 新增 `compact_jersey_rows`：把已生成的行之间的间距压缩到 20mm**：光修文字悬空只能保证"以后生成的间距正确"，已经生成的 18 行面板本身的绝对位置在创建那一刻就定死了，文字位置修正不会让面板跟着挪。因此新增独立工具：从最顶行开始（不动），后续每行整体（面板+文字+十字标记+材质标签，作为一个整体一起平移，内容/尺寸完全不碰）往上移动，移到与上一行恰好相距 20mm 为止，逐行级联。已在一次性测试文档上验证（3 行，人为设的 150mm 错误间距 → 全部变成 20.0mm，最顶行原地不动），再应用到用户真实文档：17 个间距全部从 300+mm 不等精确变成 20.0mm，18 行姓名/号码/居中偏移全部保持不变（无数据丢失）。
- [2026-09-06] **[已实现] 每行的小标签内容从"仅材质代码"改成"性别·项目·材质"（如"ЭМЭГТЭЙ · VOLLEYBALL · 3016"）**：用户要求"像 3016 那样，每件都写"，不只是页面顶部写一次。`set_jersey_material` 本身逻辑不变（还是叫 material 参数），但 app.py 调用它时把 material 参数换成了性别+项目+材质拼起来的字符串。页面顶部的 `set_batch_header` 保留，作为额外的整体概览，不替代每行的标签。

- [2026-09-06] **[已修复，真 bug] 文字居中的参照点用错了——应该对齐十字标记，不是面板几何中心**：用户发截图指出"仍然没居中"。排查后发现：十字标记（crosshair，打印时的真实对位标记）本身就不在面板的数学几何中心上（实测同一行前/背面十字标记与面板中心相差 15~65mm 不等，每行都不一样），但之前所有居中逻辑（`_apply_one`、`resize_jersey_row`、`scan_jersey_rows` 的诊断偏移）全部拿面板的几何中心当基准——文字确实"居中"了，只是居中到了错误的参照点上，跟十字标记对不齐，肉眼看就是"没居中"。已改成：每一侧优先找该侧的十字标记（Type≠6 的那个非面板形状），用它的中心作为对齐基准，找不到才退回面板几何中心。新增 `_crosshair_center_x()`/`_row_reference_centers()` 两个共用函数，`_apply_one`/`resize_jersey_row`/`recenter_jersey_texts`/`scan_jersey_rows` 四处全部统一改用。在真实文档上对全部 18 行跑了 `recenter_jersey_texts()`，修复前 17 行都有 -15~-65mm 不等的偏移，修复后全部变成 0.0。**教训：这个项目里"面板"和"打印对位标记"是两个独立摆放的物件，不能假设后者以前者为中心——凡是"居中"相关的计算都要先确认对齐的是不是真正应该对齐的东西，而不是最方便算的那个几何中心。**

- [2026-09-06] **【金规则，曾漏实现】背面姓名最大宽度 250mm，超出必须等比例缩小**：用户很早就说过"姓名最大宽度 25cm，超出时等比例缩小（不允许非等比拉伸变形）"，但一直没实现，用户翻出来质问才补上。已在 `_apply_one`（写姓名时）和 `recenter_jersey_texts`（批量修既有行时）两处实现：写完内容量到真实宽度后判断，超限则宽高按同一比例缩小。真实文档上一跑，18 行里有 **7 行**的姓名超限（MUNKUN / ARIUN JRGL / URANGOO / SERGELEN / HUSLEN / ARIUNZAYA / NOMIOO），说明这条规则漏了很久、影响面不小。
- [2026-09-06] **【真 bug，测试拦下】等比例缩小长姓名会把它变矮到 20mm 以下，从而被当成面料标签覆盖掉**：实现上面那条金规则后，在一次性测试文档上立刻暴露：一个 700mm×50mm 的长姓名缩到 250mm 宽时高度只剩 16mm，低于 `_MIN_TEXT_HEIGHT`(20mm)，于是 `set_jersey_material` 把它当成面料标签，**直接把球员姓名覆盖成了"3016"**。根因是"矮 = 面料标签"这个单一判据不够。已改成 `_is_material_label(w, h)`：必须**又矮又窄**（h<20mm 且 w<120mm）才算标签——真实标签约 24~40mm 宽，而缩过的姓名是 250mm 宽，宽度能干净地区分。`_collect`、`set_jersey_material`、`resize_jersey_row` 的重新居中三处判断已统一改用它。**教训：给某类元素加"自动缩放/自适应"能力时，要检查它会不会因此掉进别处基于尺寸的分类判据里——尺寸既是外观也是身份标识时，改尺寸就是在改身份。**

- [2026-09-06] **身体尺寸 → 面板尺寸的换算公式（用户口述，已按截图纠正过一次方向）**：表格里的 `width_cm`/`length_cm` 是身体参考尺寸，**不是**面板尺寸。正确换算是：
  - **面板宽度 = length_cm ÷ 2**（例：112 → 56）——说明表格里叫 "length" 的这一列，实际语义更接近"胸围类周长"，对折成一片就是半个周长。
  - **面板高度 = width_cm + 7**（例：68 → 75）——说明表格里叫 "width" 的这一列，实际语义更接近"衣长"，+7 是缝份/下摆余量。
  - 即 68/112 的人 → **56cm 宽 × 75cm 高**的面板（高大于宽，才是正常球衣比例）。
  第一次实现时把方向做反了（做成 75 宽 × 56 高），生成出来的面板"宽比高还大"，用户发截图指出后才改对。**教训：这两列的列名和它们的实际语义是反直觉的（"length"当宽用、"width"当高用），后续任何涉及这两列的计算都要先回来核对这条，不要按字面意思想当然。**
- [2026-09-06] **[已修复] `_panel_center_x` 因为卡 Y 方向包含判断，导致大量文字静默跳过居中**：旧版要求文字的垂直中心必须落在面板的 Y 范围内才认定归属，但 (1) 本项目形状的 Y 基准在"创建类 API"和"重定位类 API"之间本来就不一致（见本文件更早的已知约束），(2) 用户参考件的正面队名/号码本来就故意挂在面板下方几百 mm（用户确认是有意为之）。两者叠加导致垂直包含判断经常不成立、函数返回 None、调用方（`_apply_one`、`resize_jersey_row` 的重新居中）直接跳过——用户反复看到的"文字没居中"就是这个原因。已改成只用 X 方向判断归属：同一行的正/背面板在 X 上不重叠，单靠 X 就能唯一确定文字属于哪一片，不需要也不应该再卡 Y。修复后测试文档里四个文字的居中偏移全部是 0.00mm。
- [2026-09-06] **[已实现] 面料标签不存在时自动创建，不再要求用户手动加**：`set_jersey_material` 遇到某行完全没有小标签（高度 < 20mm 的文字）时，现在会在该行两个面板的左下角各自动新建一个 8mm 高的小文字标签再写入面料代码。以前只是返回 `labels_updated=0` 并在界面上提示"请先在模板行手动加一个标签"，用户重复反馈了很多次（"I am saying this like 10th time you create that little tag"）。**教训：当工具发现前置条件缺失时，先判断这个前置条件是不是自己就能安全地补上——能补就补，不要习惯性地把活推回给用户。**
...
- [2026-09-06] **[已处理，2026-09-11] 仓库里堆积了一批与本次会话工作无关、来源不明的未提交改动**：`git status` 显示 `AGENTS.md`/`CLAUDE.md`/`README.md`/`server/test_e2e.py`/`server/tools/shapes.py`/`server/tools/text.py` 已修改、`server/tools/vision.py` 未跟踪，但本次 Session 只碰过 `server/app.py`。这些改动看起来来自更早、commit 2206407（Gemini vision 接入）之后的某次未完成收尾的 Session（`.harness/.session-end-flag` 当时也确实被删除、提示过"上次 Session 可能未完成 SESSION_END"）。本次未对这些文件做审查就顺手一起提交，只提交了 `server/app.py` 自己的改动。**下次 Session 应该先看一遍这几个文件的 diff，确认内容是完整/正确的再决定提交还是丢弃**，不要让它们无限期堆积下去。
  **【2026-09-11 处理结果】**：逐一审查后发现比预期更严重——`server.py` 在 HEAD 已经注册了 `shapes.rename_shape`/`duplicate_shape(_batch)`/`duplicate_shapes`/`set_shape_locked`/`flip_shape` 和 `text.get_text_content`/`create_artistic_text` 等工具（commit 98feee3），但 `shapes.py`/`text.py` 里当时完全没有这些函数的实现——即 HEAD 处于"工具已注册但 import 会失败"的破损状态，不知道破损了多久。确认所有堆积改动（含未跟踪的 `vision.py`）AST 语法检查通过、无敏感信息（无 API Key/密码，`.env` 本就被 gitignore），内容与 `server.py` 的注册一一对应、属于同一功能（视觉预览 + 图形/文字工具补全），遂合并为一个 commit（`4c22937`）提交并推送到用户 GitHub 仓库（`Anhfurev/coreldraw-mcp`，`master`→`main`）。**教训**：MCP 工具注册（`server.py` 的 `mcp.add_tool`）必须和对应的函数实现放进同一个 commit，不能拆成两次 Session 提交，否则中间会有一段 HEAD 实际无法 import 的破损状态，且不会有任何东西提醒你（server.py 语法本身是合法的，只有真正 import 时才会报错）。
