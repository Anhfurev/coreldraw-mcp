...
- [2026-09-05] **多位数号码字符间距过大时，用 `char_spacing`（如 80，即收紧到 80%）比强行拉宽整个形状更有效**：`set_text_style(shape_id, char_spacing=80)` 直接收紧字符间距，实测能明显收窄如"42"这类数字中间的空隙；配合一个很小幅度（3%~5%）的整体宽度微调即可满足"长数字要稍微舒展但不能有明显字缝"的要求，比单纯放大宽度更贴近人的观感。
