...
- [2026-09-06] [用户反馈][待重新设计] 核对表已从"可编辑 data_editor + 下方蓝色只读预览"简化成"只有一个蓝色只读预览表"（用户明确要求"we just need blue colored table"），代价是逐格手动改值的能力、以及配套的"手动改的尺寸存进 corrections 文件、下次同身高体重直接命中"学习闭环（`_save_edited_size_corrections`/`_save_correction`）跟着一起没了（`_lookup_correction` 读取仍在工作，只是不会再有新数据写入）。用户当时的原话只谈了列顺序和颜色，没有明确表态是否知道会连带失去编辑能力——如果之后发现"AI 读错名字/号码没法在表里直接改"或者"修正不再自动积累"是问题，需要重新设计一个既能显示蓝色又能编辑的方案（比如换 streamlit-aggrid 组件，或者加一个单独的"提交修正"小表单），而不是简单地把 data_editor 加回来（那样蓝色又没了）。
