...
- [2026-09-05] [用户反馈] 新业务线：运动球衣印刷（Jersey）— 与现有门牌信号业务完全不同的应用场景。前后片各一个面板：背面=姓名+号码，正面=号码。已获取的具体规则（口述，尚未正式设计评审）：
  ...
  - 用户会在每片上手动放置"十字对位标记"（crosshair，水平+垂直线）：正面标记距顶部 21cm，背面标记距顶部 40cm（对应姓名位置基准——姓名顶部应对齐此线），两者标记线长度统一为 14cm（曾误以为背面是 40cm 长）。用户强调这是"habitually 使用但有时会不同"的经验值，不是硬性公式
  - 正面除号码外还会加队名（示例："BIRDS"），具体位置规则待用户确认
  ...
  - 下一步：用户将手动准备一份"标准参考"面片（体现上述所有规则），准备好后由 Agent 据此批量生成 10 人的球衣（不同姓名/号码/体型），要求生成速度快
  - 尚未正式过 DISCOVER/DESIGN 评审，仅在 VERIFY/临时测试中做了单件原型验证（对应 registry/_index.md 2026-09-05 记录），未来若要产品化需要走完整流程并把此条从"待评估"移到"已规划"
