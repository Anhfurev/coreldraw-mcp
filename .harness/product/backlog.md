...
- [2026-09-06] **【待修复，真 bug】`resize_jersey_row` 把一行改高之后可能侵入相邻行的空间**：`duplicate_jersey_rows` 在创建新行时，是按"这一行当时的默认尺寸（模板尺寸）"计算好与上一行的 20mm 间隙再摆放的；如果创建后又用 `resize_jersey_row` 把它改得比默认尺寸更高，多出来的高度增量到底是往哪个方向长的（CorelDRAW 的 Shape.SizeWidth/SizeHeight setter 作用在 Group 上时，是以左下角/左上角/中心哪个点为基准保持不变），目前还没有干净地验证清楚——从一次实测看锚点确实发生了偏移，但偏移量对不上"以中心缩放"或"以某个角缩放"的简单假设。如果缩放是以中心或顶部为基准，行长高之后会往下长，直接吃掉与下一行之间刚留出的 20mm 甚至更多空间，导致覆盖/重叠。**下次修复方向**：在全新空白测试文档里，专门做一个受控实验（面板初始位置记录清楚，resize 前后分别记录面板四个角的绝对坐标，而不是只看 SizeWidth/SizeHeight 和 anchor y），先精确搞清楚 CorelDRAW group resize 到底钉住哪个点不动，再决定 `resize_jersey_row` 要不要在 resize 之后再手动重新定位（比如强制把底边钉回 duplicate 时算好的原始底边位置，只允许往上长）。已把 duplicate+resize 的调用顺序从"批量 duplicate 全部再逐个 resize"改成"每个人 duplicate 一个立刻 resize 一个"（这部分改动本身是对的，能让每次 duplicate 看到上一行的真实当前尺寸），但光改调用顺序不能解决"resize 本身可能往哪个方向长"这个更底层的问题。**在这个问题查清楚并验证前，`resize_jersey_row` 和 app.py 里调用它的批量生成路径都应视为不可靠，不建议在真实生产批次上使用。**
