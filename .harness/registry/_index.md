...
[2026-09-05 21:45] FIX 球衣批量生成修复：(1) HASSAN 行大码体型缺失 resize 导致 gap 超大，重建整行并验证尺寸；(2) 2+ 位号码间距过松，改用 char_spacing=60 替代 80+width_stretch 组合（两效果互干扰），现"42"显示紧凑；(3) 确认用户需求：fabric_code 标签每件必填，性别标记（эрэгтэй/эмэгтэй）待接收，20mm 行间距锁定，RGB→CMYK 验证通过
