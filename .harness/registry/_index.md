...
[2026-09-06 18:20] FIX 补上漏掉的金规则（姓名 250mm 上限）+ 拦下一个会覆盖球员姓名的真 bug + 三个新工具 + 清理用户真实文档。用户翻出很早说过的规则质问："姓名最大宽度 25cm，超出等比例缩小"——一直没实现。补上后在真实文档上一跑，18 行里 7 行姓名超限，说明漏了很久。实现过程中在一次性测试文档上立刻暴露一个更严重的连锁 bug：等比例缩小会让长姓名变矮到 20mm 以下，掉进"矮=面料标签"的判据里，被 set_jersey_material 把球员姓名直接覆盖成"3016"。已把标签判据改成"又矮又窄"（h<20mm 且 w<120mm，真实标签 24~40mm 宽 vs 缩过的姓名 250mm 宽），三处判断统一。新增三个工具：`recenter_jersey_texts`（批量修既有行的居中+姓名超限，因为之前 _panel_center_x 的 Y 判据 bug 导致已生成的行文字没居中）、`delete_jersey_row`（删整行，用于删模板行）、`set_batch_header`（页面顶部写批次信息：性别/项目/面料）。app.py 已接上 header，生成完自动写。用户真实文档已按其要求清理：重新居中（修好 10 处偏移、7 个超限姓名）+ 删掉模板行，现在恰好 17 行 = 17 个真人，ARIUN 重复问题也随之解决。
